...
이해함으로써 각각을 어떻게  
처리하는지를 알아보도록 하겠습니다  
여러분의 프로그램에서 종종 발생하는 첫 번째 오류 유형은 구문 오류입니다  
코드에 오타가 났다던가  
...
이제 브라우저로 가서 localhost 3000을 다시 불러옵시다
...
그리고 만약 모든 요청에 적용할 미들웨어가 있다면
다른 미들웨어 위에 추가하고
이런 식으로 추가하면 됩니다 필터를 입력하지 않거나
모든 요청과 일치하는 필터를 입력하면
이 미들웨어가 항상 먼저 실행됩니다
그리고 next 함수를 호출하면 요청이 계속 전달됩니다
따라서 요청이 이어지게 해주는 미들웨어를 여기에 두고
This always runs!라는
console.log를 추가한 다음
이를 저장하고 다시 불러오면
그리고 add-product로 가면
This always runs!가 두 번 표시됩니다
이렇게 작동합니다
여기까지 미들웨어가 작동하는 방식과 이를 활용해 요청을
적절한 장소로 전달하는 방법을 알아보았습니다
]

# 64. 수신 요청 분석
[
미들웨어에 대해 자세히 알아보았으므로
이제 들어오는 요청을 실제로 어떻게 처리하고
데이터를 추출하는 방법을 알아보겠습니다
이를 위해서는 POST 요청을 처리할 수 있어야 합니다
여기 add-product에서
폼(form)이 있는 html 페이지를 반환하고 싶다고 해보겠습니다
이를 위해 다시 폼을 회신하겠습니다 덧붙여 말씀드리면
이는 다소 불완전한 html 문서입니다
이를 html과 body와 같은 태그로 둘러싸야 합니다
여기서는 읽기 쉽도록 간결하게 쓰겠지만
나중에 제대로 된 html 코드도 쓸 테니 걱정하지 않으셔도 좋습니다
이제 여기에 폼이 있고 그 안에 유형이 text이고
이름이 title인 입력값을 넣겠습니다
그리고 다시 버튼을 추가합니다 버튼의 유형은 submit입니다
이 폼을 제출하고 POST를 보내야 하기 때문입니다
캡션은 Add Product로 하겠습니다
이것이 우리의 온라인 매장에 상품을 추가할 수 있도록
해주는 폼이라고 시뮬레이션 해보겠습니다
이것이 우리의 폼이고 이 폼에는 액션이 필요합니다
요청을 보낼 경로 즉 url이 필요합니다
이름은 product로 하겠습니다 메서드는 POST이어야 합니다
이렇게 쓸 수 있습니다
이는 폼이 포함된 html 코드를 회신할 것입니다
이제 product 요청을 처리할
라우트 또는 미들웨어가 필요합니다
app.use('/product')를 입력합니다
중요한 점은 이를 이 미들웨어 전이나 후
어디에 두어도 충돌하지 않는다는 점입니다
경로에 관련된 공통점이 없고 product가 들어가긴 하지만
/add-product와 /product는 다르기 때문입니다
이 이전에만 오면 되고, 그렇지 않으면 이것이 이것보다 먼저 실행되기 때문입니다
여기 위치는 괜찮아 보입니다
그리고 또 이 세 인수를 받는 함수가 있습니다
참고로 사용할 계획이 없는 인수는 생략해도 됩니다
응답을 원한다면 첫 번째 인수는 생략하면 안됩니다
순서가 중요하기 때문입니다
하지만 세 번째 인수는 사용하지 않는다면 생략해도 됩니다
그러나 존재함을 분명히 나타내기 위해 저는 항상 입력합니다
이렇게 해서 product에 대해 실행할 이 함수를 입력했습니다
이 안에서 리다이렉트 하려 하고 지금은 들어오는 데이터를 콘솔에 기록하려 합니다
여기서 리다이렉트 하려면
res.redirect를 사용합니다 수동으로 상태 코드를 설정하고
위치 헤더를 설정하는 것보다 확실히 쉽습니다
redirect는 Express에서 추가된 또 하나의 편의성 함수입니다
여기서 redirect에 /를 입력하면
자동으로 / 경로로 리다이렉트 됩니다
물론 이게 다가 아닙니다
저는 들어오는 요청의 본문에도 관심이 있습니다
사용자가 보낸 내용을 추출하는 것입니다
Express.js에는 이를 위한 편의성 기능이 있습니다
console.log(req.body)를 여기에 입력합니다
이는 Express에서 추가된 새로운 필드로 무엇이 들어있는지 보겠습니다
이제 이를 저장하고 /add-product로 돌아가면
입력 필드가 보일 것입니다
여기에 Book을 추가하고 Add Product를 누르겠습니다
/로 리다이렉트 됩니다
잘 작동함을 볼 수 있고 콘솔에는 undefined가 표시됩니다
다른 콘솔 로그를 삭제해서
로그를 정리하겠습니다
이것도 삭제해도 됩니다
이제 거의 다 왔습니다 여기에 undefined가 보이는 이유는
req.가 이 본문에 편의 속성을 주지만
기본적으로 req.는 들어오는 요청의 본문을
분석하려 하지 않기 때문입니다
이를 위해 분석기를 등록해야 하는데 또 다른 미들웨어를 추가해서 구현합니다
이는 일반적으로 경로 처리 미들웨어 이전에 두는데
요청이 어디로 향하든 본문 분석이 이루어지도록 하기 위해서입니다
여기서 들어오는 요청 본문을 분석하고자 합니다
이를 위해 제3자 패키지를 설치해야 합니다
npm install --save를 실행해서 이를 설치합니다
우리 코드에서 사용할 패키지이며
실무에서 중요하기 때문입니다
save dev가 아니라 save로 하고 이름은 body-parser 입니다
사실 이는 Express에 기본적으로 포함되어 있습니다
사용자들이 다시 그러길 원했기 때문입니다
과거에 포함되어 있다가 빠졌고 다시 추가되었습니다
저는 추천 사용법대로 제3자 패키지를 사용하겠습니다
이 패키지가 향후 Express에서 다시 빠지더라도
제가 가르쳐드리는 이 코드가 여전히 작동하도록 하기 위해서입니다
새로운 body-parser 패키지를 설치했으므로 여기에 임포트 할 수 있습니다
bodyParser 상수에 저장하겠습니다 언제나처럼 이름은
원하는 대로 하시면 됩니다 패키지 이름은 body-parser입니다
이제 bodyParser을 호출해서 여기서 사용할 수 있어
이 객체를 사용하게 됩니다 그리고 .urlencoded를 입력합니다
이는 실행해야 하는 함수로
옵션을 입력해 설정할 수 있지만 여기선 그럴 필요가 없습니다
이 코드는 미들웨어를 등록해 줍니다
따라서 마지막에 이 함수는 이러한 미들웨어 함수를 내놓습니다
이 부분은 마지막의 이러한 함수를 분석해 줍니다
눈에 보이진 않을지라도 이 패키지는 최종적으로
이 미들웨어 함수의 끝에서 next를 호출해서
요청이 우리 미들웨어에 도달하도록 해줍니다
하지만 그 이전에 이전 코어 섹션에서
수동으로 해야 했던 요청 본문 분석을 수행합니다
물론 파일, json 등의 가능한 모든 본문을
분석해 주지는 않습니다 그러나 여기 있는 것과 같이
폼을 통해 전송된 본문은 분석해 줍니다
이번 강의 과정에서 다루겠지만 파일과 같은 다른 본문의 경우에는
다른 분석기를 사용하면 됩니다 이러한 점이 Express.js의
높은 확장성을 보여줍니다 뭔가 필요한 게 있다면
코드 한 줄로 쉽게 설치할 수 있습니다
임포트까지 세면 두 줄이겠군요
이렇게 해서 이 console.log의 원하는 출력값을 얻을 수 있습니다
이제 서버를 재시작하겠습니다
참고로 새로운 패키지를 설치했으면 재시작해야 합니다
nodemon의 자동 재시작 기능에 의존하지 않는 것이 좋습니다
그리고 여기 경고가 뜨는 걸 보니 하나 설정할 것이 있습니다
extended를 false로 설정해서 여기에 설정 옵션을 입력해야 합니다
이는 비표준 대상의 분석이 가능한지를 나타냅니다
이를 우리의 용도에 맞게 추가하면
본문 분석기가 활성화되었습니다
이제 다시 해보겠습니다 add-product로 돌아가서
다시 Book을 추가합니다 Add Product를 누르면
리다이렉트 됩니다 이제 출력을 보면 키-값 쌍이 있는
JavaScript 객체를 얻었습니다 이전에 split 함수를 사용하고
수동으로 배열을 만드는 등의 방식보다
더 쉽게 이 값을 추출할 수 있었습니다
이제 객체 하나를 출력값으로 얻었는데 여기 입력에서 정의한 키, 즉 이 이름과
사용자가 입력한 값이 보입니다
이는 이전의 맞춤식 접근법에 비해 확실히 더 간단합니다
이제 사용자들이 전송하는 데이터를 처리할 수 있고
추후에 해보겠지만 데이터베이스에 저장할 수 있으며
응답을 보여주는 등 원하는 작업을 모두 할 수 있습니다
하나가 빠졌습니다 지금 이 부분은
들어오는 데이터 요청에 대해서도 실행됩니다
그러나 우리는 POST 요청만 듣고 싶습니다
이를 어떻게 처리하면 될까요?
]
...
[
본문 분석기 패키지의 도움을 받아
요청 본문을 깔끔하게 분석해 보았습니다
그러나 언급한 대로 현재로서는 이 미들웨어가 POST 요청뿐만 아니라
GET 요청에 대해서도 실행됩니다 이를 어떻게 처리하면 될까요?
app.use 대신에 app.get을 사용할 수도 있습니다
이는 기본적으로 app.use와 동일하고 app.use와 같은 구문입니다
경로를 사용하거나 사용하지 않을 수 있고 들어오는 GET 요청에만 작동합니다
이는 경로 필터가 아닌 다른 형태의 필터라고 할 수 있습니다
app.get은 GET 요청을 걸러낼 수 있게 해줍니다
그리고 같은 페이지에 들어오는 POST 요청을 걸러주는
app.post가 있습니다 이 단어 하나를 바꿈으로써
이 미들웨어는 이제 이 경로에 해당되는
들어오는 POST 요청의 경우만 실행되고 GET 요청에는 반응하지 않습니다
이제 이를 저장하고 /product로 가보면
Hello from Express!를 볼 수 있습니다
따라서 /product를 입력해도 GET 요청이므로 여기 도달하지 않습니다
그러나 add-product에 있는 폼을 통해 POST 요청을 보내면
여기서 Book 2를 추가하면 이런 출력값을 얻게 됩니다
필터를 통과해서 이 미들웨어에 도달했음을 확인할 수 있습니다
이는 미들웨어 함수를 활용하는 또 다른 방법입니다
모든 http 메서드에 반응하는 use 대신에
get이나 post를 사용해서 해당 요청만 걸러낼 수 있습니다
추가적으로 delete, patch, put도 있지만
이들은 추후 강의에서 사용하게 될 http 단어들로
일반적인 html 문서에서는 사용하기 어렵습니다
]

# 66. Express 라우터 사용
[
이번 모듈이 거의 끝났습니다 Express.js의
핵심 개념에 대해 이미 많은 것을 배웠습니다
이후 강의들이 모두 여기서 배운
Express.js의 작동 원리를
기반으로 진행되기 때문에 이는 매우 중요한 모듈입니다
여기 보이는 우리 더미 앱은 매우 간단하긴 하지만
지금까지 모든 코드를 하나의 app.js 파일에
기록함에 따라 파일 크기가 커지고 있습니다
물론 이 정도 규모의 앱에 대해서는 문제가 되지 않습니다
여전히 꽤 작은 편이기 때문입니다 그러나 일반적으로는
라우팅 코드를 여러 파일로 나누는 것이 좋습니다
우리의 로직을 여러 파일로 내보내서 이 파일로 임포트 하려 합니다
이는 충분히 가능한 일로 이러한 함수를 내보낼
파일을 직접 만들 수도 있지만 Express.js는
라우팅을 다른 파일에 위탁하는 편리한 방법을 제공합니다
이를 위해 먼저 폴더 구조를 약간 변경하겠습니다
routes라는 이름의 새로운 폴더를 생성하겠습니다
이렇게 하지 않고 다른 이름을 붙여도 되지만
흔한 관행에 따른 것입니다
다양한 경로와 http 메서드에 대해 실행할
라우팅 관련 코드를 파일에 담아서
routes 폴더에 저장하도록 하겠습니다
그리고 우리는 온라인 매장 구현을 목표로 하고 있으므로
admin.js라는 라우팅 파일을 생성하겠습니다
이는 매장 관리자가 담당하게 될
제품 생성을 처리하는 라우팅 파일입니다
또한 shop.js 파일도 만들겠습니다
여기엔 사용자가 보게 될 내용을 담겠습니다
여기서 매장 전체를 구현하지는 않겠습니다 향후 강의와 모듈에서
데이터베이스와 같은 멋진 기능들을 활용해서
천천히 만들어 갈 예정입니다
그러나 적어도 우리 코드를 이 파일로 옮기기 시작할 수는 있습니다
add-product 경로와
이 product post 요청은 admin.js에 넣어야 합니다
이들은 관리자가 도달해야 할 라우트입니다
여기 보이는 일반 라우트는 shop.js 파일에 넣어서
첫 페이지를 방문한 사용자가 볼 수 있도록 해야 합니다
이를 실현하기 위해 Express.js에서 제공하는
편리한 기능을 사용하려면 이 파일에 들어가서 express를
다시 임포트합니다 일반적으로 여러 파일로 임포트 하게 되며
그 후 Router이라는 기능을 사용할 수 있습니다
소문자 r로 만들어도 되고 이름은 마음대로 하셔도 좋습니다
그리고 express.router을 호출해서 생성합니다
이는 실행하게 될 함수입니다
이 라우터는 다른 Express 앱에 연결되어 있거나
꽂아 넣을 수 있는 Express 앱과 같으며
여기로 내보낼 수 있습니다
따라서 여기에 module.exports = Router을 입력합니다
지금은 별다른 일이 없이
이제 이 라우터를 사용해 대상을 등록해야 합니다
다른 이름과 일관성을 유지하는 의미에서
소문자 r로 변경하겠습니다
그러나 여기는 대문자 R을 써야 합니다
이제 이 라우터를 사용해서 모든 요청에 대한 use 함수,
GET에 대한 get 함수, POST에 대한 post 함수 등을 정의할 수 있습니다
따라서 app.js 파일로 돌아가서
이 두 관리자 라우트를 잘라낸 다음
admin.js 파일에 붙여넣고
app을 router로 바꾸기만 하면 됩니다
라우터를 내보냈으므로 이 라우터에는 이 두 라우트가
등록되어 있습니다 여기로 내보냈기 때문입니다
그리고 이것이 우리가 이 라우트를 등록한 객체입니다
라우터가 app.use
또는 app.get 등의 함수와
완전히 동일한 기능을 수행하므로 다른 코드를
그대로 사용할 수 있습니다
상품을 추가하고 이 폼을 회신하는 데는 GET 요청만을
처리하고자 하므로 이를 get으로 변경하겠습니다
그리고 이를 여기로 내보내고 나면 이를 app.js 파일로 임포트합니다
이를 위해 express와 분리된 임포트를 위에 추가하겠습니다
제 파일임을 확실히 하기 위해서입니다
이름은 adminRoutes로 하지만 원하는 대로 정하셔도 됩니다
여기에 routes 폴더의 상대 경로가 필요합니다
그리고 그 안의 admin 파일을 지정해 줍니다
전에 언급했듯이 .js는 자동으로 추가되므로 생략해도 됩니다
이제 이 router 객체를 임포트 하게 되고
이 router 객체에는 이러한 루트가 등록되어 있습니다
이 router의 장점은 바로 이것이 유효한 미들웨어 함수라는 점입니다
따라서 app.use를 호출해서 이렇게 adminRoutes를
입력할 수 있습니다 함수처럼 호출하지 않고
괄호 없이 객체 자체를 입력하면 됩니다
바로 이 파일로 내보낸 router 객체입니다
이렇게 사용할 수 있습니다 이제 이 미들웨어를 통해
요청을 처리할 때 자동으로 admin.js 파일의
라우트를 고려하게 됩니다
전과 같이 순서가 중요합니다 따라서 이를 이 미들웨어 후에 두면
절대 도달하지 못한다는 사실은 변하지 않습니다
이제 정면 라우트도 같은 작업을 할 수 있습니다
shop.js 파일로 돌아가서 영상을 일시 정지하고
스스로 해보시길 추천드립니다
admin.js에서 했던 것처럼 express.Router을 사용해서
이를 구현해 보시길 바랍니다 성공하셨나요?
먼저 require로 express를 임포트 하겠습니다
그 다음 express.Router을 호출해서
router 객체를 생성합니다
여기서 router을 내보냅니다
그 다음 use.app을 사용하거나 복사한 내용을 붙여 넣어서
app을 router로 변경하고 필요하다면 get을 사용합니다
이렇게 할 필요 없이 use를 사용하는 방법도 존재하지만
여기서는 GET 요청만을 다루겠습니다
이제 admin.js 아니 app.js로 돌아가서
모든 라우트를 임포트합니다 임포트 순서는 중요하지 않습니다
shopRoutes이고 routes 폴더의 shop.js 파일에 require 합니다
이제 여기서는 다시 순서가 중요한데 두 번째를 등록합니다
이를 저장하고 add-product를 다시 불러오면 제대로 작동합니다
여기서 이해해야 하는 중요한 점이 있습니다
이 둘의 위치를 바꿔서
shopRoutes를 위에 둔 다음 다시 불러와도
제대로 작동하고 이 라우트로 가지 않습니다
이는 get을 사용했기 때문입니다
get, post 등은 경로와 정확히 일치하는 요청만을 처리합니다
전처럼 모든 들어오는 http 메서드를 처리하는 use를 사용한 채로
다시 불러오면 Hello from Express!가 보입니다
따라서 이 매칭은 라우터를 통해서가 아니라
get을 통해 이루어집니다 그리고 이는 전에
app.js 파일에서 했던 것처럼 앱 방식으로 했더라도
동일했을 것입니다
따라서 get은 GET 메서드임을 확인할 뿐만 아니라
정확히 이 경로임을 확인합니다
따라서 이제 무작위 경로를 입력하면 오류가 발생합니다
이제 이를 처리할 단일 미들웨어가 존재하지 않기 때문입니다
이제 라우트를 이런 식으로 나누어 등록해서 설정해 두었습니다
언급했듯이 여기에서 순서를 신경 쓰는 것은 여전히 좋은 습관입니다
이 경우에는 순서에 관계없이 제대로 작동한다 하더라도 말입니다
그러나 만약 무언가를 다시 use로 변경하면
다시 순서가 중요해지므로 처음부터 신경 쓰는 것이 좋습니다

# 67. 404 오류 페이지 추가
[
지난 강의에서는 express.Router을
사용해 보았습니다 이번에는 처리되지 않는
몇몇 요청이 포함되어 있습니다
여기에 무작위 문자열을 입력하면 오류가 발생합니다
일반적으로 이런 경우에는 404 error 페이지가 나타납니다
app.js file 파일로 돌아가서
미들웨어 또는 Express가 미들웨어를 사용해
요청을 처리하는 방법을 활용하면 됩니다
요청이 위에서 아래로 내려간다는 사실을 기억하시길 바랍니다
자신을 처리하는 미들웨어를 발견하면
거기로 들어가게 됩니다 예를 들어 여기 /는
라우터의 get 메서드에 도달하게 됩니다 또한 next를 호출하지 않기 때문에
다른 미들웨어는 실행되지 않습니다
그러나 만약 적당한 미들웨어가 여기에 존재하지 않는다면
맨 아래로 내려가게 되고 요청은 처리되지 않습니다
따라서 404 오류 페이지를 전송하려면 맨 아래에 모든 요청을 처리하는
미들웨어를 추가하면 됩니다 경로 필터는 필요하지 않지만
/를 추가해도 좋습니다 어차피 기본값이기 때문입니다
그리고 req, res, next를 입력하면 됩니다
여기선 use를 사용했으므로
GET 요청만이 아니라 모든 http 메서드를 처리하게 됩니다
그다음 Page not Found 같은 코드를 send 하면 됩니다
이렇게 더미 html 문서를 만들고
404 상태 코드를 설정할 수도 있습니다
send 이전에 status라는 또 하나의 메서드를 연결해 주면 됩니다
여기서만 사용할 수 있는 것은 아니고
여기나 admin.js 파일에서도 사용할 수 있습니다
전송 전에 항상 status나 setHeader을 호출할 수 있습니다
모든 메서드 호출을 연계할 수 있고 마지막이 send이기만 하면 됩니다
여기서 status를 호출해 상태 코드를 설정하는데
Express.js에서 추가하는 또 다른 편의 기능입니다
페이지를 찾을 수 없는 경우의 일반적인 코드인 404로 설정하겠습니다
이렇게 하고 저장한 다음 더미 경로를 다시 불러오면
Page not found가 나타납니다 반면 add-product를 입력하면
폼이 나타나고 제대로 작동하지만 나머지는 변경되었습니다
]

# 68. 경로 필터링
[
Express Router에는 또 하나의 주목할 만한 기능이 있습니다
이제 모든 경로를 파악했고 여기엔 아웃소싱 라우트가 있습니다
아웃소싱 라우트들은 시작 경로를 공유하는 경우가 있습니다
모든 admin 라우트가 admin/add-product로 트리거되고
여기도 admin/add-product로 트리거 된다고 해보겠습니다
메서드가 get과 post로 다르기 때문에 경로를 반복해도 됩니다
이 둘은 서로 다른 두 라우트가 됩니다
여기서 이미 메서드가 다르면 같은 경로를 사용할 수 있다는
중요한 사실을 알 수 있습니다 물론 여기 폼에서
add-product를 가리켜야 합니다 또 하나 중요한 사실이 있습니다
이러한 라우터 파일의 경로가
같은 구간에서 시작하는 경우
여기 라우트에서 해당 구간을 빼서
app.js 파일로 간 다음 여기에 추가해서
해당 구간을 필터로 추가할 수 있습니다
/admin으로 시작하는 라우트만
adminRoutes 파일로 들어가게 됩니다
뿐만 아니라 여기와 Express.js에서
이러한 라우트를 매칭하려 할 때 url에서 이 /admin 부분을
생략하거나 무시하게 됩니다
이제 /add-product는 /admin/add-product 라우트에 매칭됩니다
/admin이 생략되었기 때문입니다
이를 실제로 보여드리겠습니다 add-product 라우트를
새로 고침하면 페이지가 존재하지 않으므로 Page not found가 나타납니다
이제 이는 /admin/add-product이고 실제로 여기 폼이 있습니다
그리고 여기 다시 Book을 추가하고 Add Product를 누르면
폼 액션이 /add-product로 이어지므로 Page not found가 나타납니다
이 역시/admin/add-product이어야만 합니다 도달하려는 admin.js 파일은
앞에 /admin이 있는 요청을 통해서만 접근할 수 있기 때문입니다
다시 한번 해보겠습니다
/admin/add-product로 가서
다시 Book을 추가합니다
이제 제대로 리다이렉트 되었고 여기에 기록되었음을 볼 수 있습니다
따라서 여기 app.js에 있는 필터를 통해
주어진 파일의 모든 루트가 app.js 파일로 위탁하는 데 사용할
공통 경로를 앞에 추가할 수 있습니다
모든 루트에 대해 이를
반복해서 입력할 필요가 없습니다
이 경로는 함축적으로
admin/add-product에서 도달하게 되며 이 라우트도 마찬가지입니다
이 경우는 POST 요청, 이 경우는 GET 요청입니다
이는 이해하기 조금 어려울 수도 있는데
이러한 요청이 어떻게 통과해서
어떻게 이 파일에 도달하는지는 꼭 이해해야 하는 핵심 사항입니다
필터에 의해 /admin으로 시작한 경우 해당 파일에서 /admin 부분은
다시 확인하지 않고 두 번째 부분만 확인해서
여기 또는 여기 보이는 request 라우트에 도달하게 됩니다
]

# 73. 내비게이션을 위한 헬퍼 함수 사용
[
스타일링을 시작하기 전에 shop.js와 admin.js에서  
루트 폴더로 이동하는 방법에 대해 하나만 짚고 넘어가겠습니다  
먼저 여기에 ../ 대신 ..를 사용할 수도 있습니다  
윈도우와 Mac에서 작동하지만 더 선호되는 방식입니다  
경로를 설정할 때 구분자가 무엇인지를  
가정하지 않기 때문입니다  
이렇게 하고 돌아가서 add-product로 가면  
여전히 작동합니다  
다만 좀 더 깔끔한 방법으로 해냈습니다  
그러나 이를 구현할 수 있는 더 좋은 방법이 있습니다  
helper 함수의 도움을 받아 상위 디렉터리를 얻을 수 있습니다  
이를 위해 여기에 helpers 또는 util이라는 새로운 폴더를 생성합니다  
저는 util로 하겠지만 원하는 대로 하시면 됩니다  
그리고 여기에 path.js 파일을 추가합니다  
이것이 전역 모듈과 충돌한다 해도 상관없습니다  
어차피 다른 방식으로 임포트 할 것이기 때문입니다  
이제 여기에 module.exports를 입력하고  
상위 디렉터리로 가는 경로를 구축하는데  
도움을 주는 함수 하나를 내보내고자 합니다  
실제 함수라기보단 변수라고 하겠습니다  
먼저 여기에 require('path')로  
전에 했던 것처럼 경로를 임포트합니다  
그 다음 여기에 다른 함수를 사용할 건데 join이 아니라  
dirname입니다 오른쪽 도움말에 보이듯이  
dirname은 경로의 디렉터리 이름을 회신합니다  
이는 꽤 유용한 기능으로 이를 사용하면 디렉터리 이름을  
알아내고자 하는 디렉터리나  
파일명만 알면 됩니다  
여기엔 전역 프로세스 변수를 사용하면 되는데  
이 또한 모든 파일에 사용 가능한 변수로  
임포트 할 필요가 없습니다  
그리고 여기엔 mainModule 속성이 들어갑니다  
이는 애플리케이션을 시작한 주요 모듈을 나타냅니다  
바로 여기 app.js에서 만든 모듈입니다  
이제 filename을 호출해서 이 모듈이 어떤 파일에서  
시작되었는지 알아낼 수 있습니다  
다른 말로 하면 이 코드는 우리 애플리케이션이  
실행될 수 있도록 해주는 파일의 경로를 알려줍니다  
이 파일명을 dirname에 입력해서 그 디렉터리의 경로를 알아낼 수 있습니다  
이를 통해 여기 이 파일에서 임포트 할 수 있습니다  
확인하기 쉽도록 제 임포트를 다른 줄과 분리해서 추가하겠습니다  
그리고 이를 rootDir로 명명하겠습니다  
이름은 마음대로 하셔도 좋습니다 require에 이를 입력해서  
util로 한 단계 올라가고 그 다음은 path를 입력합니다  
그리고 최종적으로는 이 루트 디렉터리를  
여기에 입력하고자 합니다 따라서 이 경로를 생성할 때  
루트 디렉터리부터 시작하겠습니다  
이제 여기서 이 페이지를 다시 불러오겠습니다  
여전히 작동합니다 결국 루트 디렉터리 경로를 구축하는  
깔끔한 방법을 사용한 셈이므로 당연히 작동해야 합니다  
shop.js에서도 같은 작업을 반복합니다  
util 폴더의 path 파일에서 rootDir을 임포트하고  
dirname ..을 rootDir로 변경합니다  
기존의 접근법을 계속 사용해도 되지만  
이것이 더 깔끔한 방법인데다  
사용하기도 간단하고  
모든 운영 체제에서 작동하며  
항상 루트 파일 경로를 알려줍니다  
...
# 74. 페이지 스타일링
[
우리 html 페이지는 심심해 보입니다 물론 이 강의는 CSS 강의가 아닙니다
참고로 CSS에 대해 더 배우고 싶으시다면 제 강의가 있습니다  
이 강의가 CSS 강의는 아니지만 CSS를 Node.js 앱에서  
어떻게 서비스하는지를 이해하는 것은 중요합니다  
일반적으로 앱에는 CSS와 JavaScript 코드가 있기 때문입니다  
간단하게 시작해 보겠습니다  
지금 슬래시 페이지, 즉 shop.html가 열려있습니다  
head 섹션에 스타일 태그를 사용해 여기에 스타일링을 추가해 보겠습니다  
이를 계속 사용하지는 않겠으며 이는 그저  
작업을 시작하고 자동으로 업데이트하기 위한 것입니다  
외부 스타일 임포트는 현재는 작동하지 않기 때문입니다  
이전과 마찬가지로 스타일을 저와 함께  
작성하고 싶지 않으시면 이 강의를 건너뛰셔도 됩니다  
다음 강의에 결과를 첨부해 두겠습니다  
그게 아니라면 같이 해보겠습니다  
헤더부터 스타일링을 시작해 보겠습니다  
해당 요소를 직접 타겟팅하는 대신  
이것이 제 메인 내비게이션 또는 메인 헤더가 될 것입니다  
main-header 클래스를 부여하겠습니다  
이게 main-header__nav가 되며  
이 이상한 CSS 이름이 궁금하시다면 BEM이라는 스타일링 체계입니다  
CSS 강의에서 더 자세하게 다룹니다 물론 CSS BEM을 검색하셔도 됩니다  
이중 클래스명을 피하기 위해 클래스를 명명하는 특정한 방식입니다  
이것이 제  
main-header__item-list 입니다  
그리고 여기는 main-header__item 입니다  
물론 이름은 마음대로 정하셔도 됩니다  
이들은 스타일링을 도와줄 CSS 클래스입니다  
이제 헤더부터 시작하겠습니다  
헤더의 너비는 100%로 해서 페이지 전체를 차지하게 하고  
높이는 56 픽셀로 하겠습니다  
또는 rem 단위를 사용하고 싶으시다면  
3.5rem으로 할 수 있겠습니다  
이제 멋진 배경색을 입혀 보겠습니다  
여기선 자유롭게 선택하시면 됩니다 빨간색을 고르진 않을거고  
이 위에 올리면 색상 선택기에서 원하는 색을 고를 수 있습니다  
저는 노란 황금빛의 주황색을 고르도록 하겠습니다  
이런 색이 되겠습니다  
rgb를 클릭하면 헥스(hex) 코드로도 변환해 줍니다  
이 색을 사용하겠습니다 물론 클래스는 header이 아니라  
main-header입니다 이제 이를 저장하고 다시 불러오면  
이렇게 보입니다 좀 나아 보이는군요  
보시면 본문에 여백이 좀 있는데  
내외부 여백이 모두 없도록 정의하겠습니다  
제 생각에는 외부 여백 같습니다 이를 추가하면 좀 더 나아보입니다  
위에 여전히 여백이 좀 있습니다  
이는 여기의 목록에서 오는 것으로 여백 호출이 작동하고 있지 않습니다  
이 역시 제거하겠습니다  
main-header__item-list를 스타일링하고 있습니다  
이를 복사하고 여기에 클래스를 추가합니다  
list-style을 none으로 하고 내부 및 외부 여백을 모두 없앱니다  
좀 더 나아보입니다  
물론 item이 볼품없어 보입니다  
이에 대해서는 여기서 display를 flex로 변경해서  
목록에 대해 flexbox를 사용하도록 하고  
item에 스타일을 약간 추가하겠습니다  
지금은 내부 및 외부 여백을 제거하고 다시 불러옵니다  
flexbox로 인해 옆으로 배치되었습니다  
이제 내부 링크를 스타일링 해보겠습니다  
main-header__item의 모든 중첩 앵커 태그는  
꾸미기는 없고 색은 흰색으로 하겠습니다 좀 나아 보이는군요  
수직 가운데 정렬도 좋아 보입니다  
다시 main-header로 돌아갑니다 아니, 그게 아니라  
main-header__nav로 갑니다 이에 대한 클래스도 추가합니다  
그리고 먼저 높이를 100%로 하겠습니다  
main-header의 하위 요소이므로  
전체 높이를 차지해야 하기 때문입니다 이 역시 display를 flex로 해줍니다  
align-items: center으로 item을 수직 가운데 정렬합니다  
이를 저장하고 페이지를 다시 불러오면  
가운데로 왔습니다  
이제 main-header에 여백을 추가하겠습니다  
여기로 가서 padding을 추가합니다 상하로는 필요 없지만  
좌우로는 1.5rem 정도로 해줍니다 그럼 이렇게 됩니다  
휴대전화에서는 이렇게 보입니다 제가 보기엔 괜찮습니다  
글로벌 폰트도 변경해 보겠습니다 body에서 font-family를  
지금은 sans-serif로 설정하겠습니다  
제 생각에는 serif 폰트보다 조금 낫습니다  
그리고 내비게이션 항목 사이에 여백이 좀 필요해 보입니다  
main-header__item으로 가서  
이렇게 좌우에 1rem의 여백을 추가하겠습니다  
지금은 이 정도면 됐습니다  
링크에 호버링 효과를 조금 넣어보겠습니다  
여기로 가서 main-header__item a: hover,  
main-header__item a: active를 입력하고  
액티브 클래스가 붙은 앵커에 적용할 규칙을 정의하겠습니다  
여기서 색을 정할 수 있는데  
여기서 노란색 계열을 사용했으니  
파란색 쪽으로 가보면 좋을 것 같습니다  
이 보라색도 좋아 보입니다  
어떤 모습일지 보겠습니다  
나쁘지 않은 것 같네요 이걸로 하겠습니다  
이제 이 링크가 활성화된 shop.html에  
이러한 active 클래스 규칙이 생겼으므로  
여기에 active 클래스를 추가할 수 있습니다  
그리고 추후 강의에서 이를 동적으로 설정하는 방법을 배우게 됩니다  
이제 이게 활성화되었고 이제 main 섹션 차례입니다  
이 역시 스타일링 해보겠습니다  
여기서는 main에 대한 일반 규칙을 정의하겠습니다  
모든 방향에 1rem의 여백을 두겠습니다  
이게 제가 원한 스타일입니다  
이제 admin/add-product 페이지도 스타일링 해보겠습니다  
shop.html 파일의 스타일 객체 전체를  
add-product.html 파일에 복사하기만 해도 꽤 나아짐을 볼 수 있습니다  
add-product에서 변경해야 될 것이 있는데  
물론 이 모든 클래스를 추가해야 합니다 이 header를 복사하시면 됩니다  
나중에 여러 파일의 재사용 방법도 배울 예정입니다  
이 header을 복사합니다  
참고로 제가 종종 사용하는 자동 서식 기능은  
Preferences의 Keyboard Shortcuts에서  
format document를 검색하면 찾을 수 있습니다  
여담으로 이게 제가 사용하는 단축키입니다  
이제 여기서 active 클래스 역시 변경해야 합니다  
여기 Shop 링크에는 없지만 add-product 링크에는 필요합니다  
이렇게 하고 페이지를 새로 고침하면 이제 훨씬 나아 보이는군요  
이제 폼을 작업해 보겠습니다  
이를 위해 form으로 가서  
product-form 클래스를 줍니다 이름은 원하는 대로 하시면 됩니다  
div를 추가하고 form-control 클래스를 준 다음  
이 div에 이 입력값을 넣고 title에 대한 label도 추가합니다  
Title을 label로 하겠습니다  
그리고 이 입력값의 id를 title로 해서  
label과 일치하도록 합니다 접근성 사유 때문입니다  
이제 이 또한 스타일링 해보겠습니다  
form-control를 사용하겠습니다  
바로 이 div에 추가한 클래스입니다  
form-control에 포함된 레이블이나 입력값이  
block 유형의 display를 사용해서  
전체 너비를 차지하도록 하려고 합니다  
그리고 input도 스타일링 하겠습니다  
여기 input은 1픽셀의 테두리를 가지도록 하고  
헤더에서 사용한 노란색을 다시 사용하겠습니다  
폰트는 inherit 해서  
기본 폰트를 사용하도록 합니다  
2픽셀 정도의 작은 테두리 반경을 지정하고  
이제 어떤 모습일지 보겠습니다  
다시 불러오면 나쁘지 않습니다  
이제 버튼도 스타일링 해보겠습니다  
이를 위해 일반적인 버튼 스타일을 설정하겠습니다  
기본 폰트를 inherit 하고  
1픽셀의 테두리를 밝은 회색으로 하고  
배경색은 흰색으로 하겠습니다  
테두리를 검은색으로 하겠습니다  
테두리 반경은 3픽셀로 하겠습니다  
어떤 모습인지 보겠습니다  
괜찮아 보입니다만 노란색을 다시 사용하겠습니다  
텍스트를 그 노란색으로 하겠습니다  
그리고 여기에 더해 pointer 커서를 할당해서  
이 위에 올리면 손 모양 커서가 되도록 하겠습니다  
마지막으로 form-control div와 버튼 사이에 공간을 좀 두려고 합니다  
따라서 form-control 자체에서  
상하에 1rem의 여백을 추가하고  
좌우는 0으로 하겠습니다  
그러면 이렇게 됩니다  
그리고 버튼의 경우에는  
읽기 쉽도록 보라색으로 하겠습니다  
또한 버튼에 대해 hover 및 active 상태도 할당하겠습니다  
활성 상태일 때는 배경색을 보라색으로 하고  
텍스트 색을 흰색으로 하겠습니다  
이제 이렇게 보입니다  
나쁘지 않아 보이는군요  
이대로 하겠습니다 물론 더 추가할 수 있고  
강의가 진행됨에 따라 계속해서 작업해 나가겠지만  
지금은 이 정도로 충분합니다  
이제 폼 전체를 가운데로 정렬하겠습니다  
폼에 product-form 클래스를 주었습니다  
여기에서 너비를 30rem으로 하고  
휴대전화로 볼 경우를 고려해서 최대 너비는 100% 또는 90%로 하겠습니다  
여백은 auto로 하겠습니다 자동으로 수평 가운데로 맞춰집니다  
이제 input 등이 사용 가능한 공간을 차지하는지 확인합니다  
이를 20으로 줄이겠습니다 30은 너무 과한 것 같습니다  
그리고 input에 대한 form-control에서  
너비를 100%로 하겠습니다  
훨씬 나아 보이는군요  
생각해 보니 이 흰색 텍스트가 맘에 들지 않습니다  
header로 가서 흰색 링크 텍스트를  
검은색으로 변경하고 어떤 모습일지 보겠습니다  
이게 더 나아보입니다  
그러니 이렇게 정하고  
shop.html에도 추가해서 검은색으로 바꿔줍니다  
그리고 보라색이 좀 더 어두운 게 좋을 것 같습니다  
hover 및 active 스타일로 가서  
더 어두운 보라색으로 변경하겠습니다 그리고 모든 보라색을 이렇게 변경합니다  
빠른 작업을 위해 IDE의 다중 선택 기능을 사용하고 있습니다  
검색해서 변경해도 됩니다 다시 불러옵니다  
좀 더 나아보입니다  
언제든 원하는 대로 변경하셔도 됩니다 지금이 CSS 강의는 아니지만  
사용 가능한 기본적인 스타일링을 추가하고자 했을 뿐입니다  
이제부터 저는 이 스타일을 사용하겠습니다 물론 계속 조정해 나갈 예정입니다  
이제 또 하나 해야 할 일은 shop.html 파일에서  
이 스타일을 복사해서 404.html 파일에 넣는 것입니다  
거기에 헤더를 복사해 넣을 것입니다  
Page Not Found! 만이 아니라  
헤더도 나타나게 하고 싶기 때문입니다 따라서 만약에  
존재하지 않는 라우트를 입력해도 여기에 헤더가 표시되도록 해서  
이 라우트에서 나갈 수 있도록 하겠습니다 url에 오류가 있어서 지금은 안되는데  
(첨부된 코드에서는 이미 수정되었습니다) 여기가 admin/add-product가 돼야 합니다  
이제 이를 폼을 포함해서  
모든 곳에서 고치도록 하겠습니다  
이제 다시 불러오면 원활하게 이동할 수 있습니다  
Book을 추가해 보면 잘 작동합니다  
이렇게 스타일링을 해보았습니다 스타일링에 대한 기본적인  
html 코드를 살펴보았습니다 문제는 이 모든 것이  
html 파일 안에 있는 것입니다 외부 파일을 사용할 수 있으면  
좋을 것 같지 않나요?  
]